...
//   for loading for the image
  ...
  const handleSubmitEvent = async (e) => {
    ...
    try {
        const response = await axios.post("http://localhost:3001/evently/events", {
            clubName, 
            eventName,
            eventLink,
            fee,
        ...
        if (response.data) {
            ...
            setClubName(""); // Set club name
            setEventName(""); // Set event name
            setEventLink(""); // Set event link
            setFee(null); // Set fee
            setDeadlineDate(""); // Set deadline date
            setStartDate(""); // Set start date
            setEndDate(""); // Set end date
            setEventLocation(""); // Set event location
            setStartTime(""); // Set start time
            setEndTime(""); // Set end time
            setPrizePool(null); // Set prize pool
            setFirstPrize(null); // Set first prize money
            setSecondPrize(null); // Set second prize money
            setThirdPrize(null); // Set third prize money
            setTags(""); // Set tags
            setNoOfCredits(null); // Set number of credits
            setTypeOfCredits(""); // Set type of credits
            setClubLogo(""); // Set club logo
            setEventPoster(""); // Set event poster
            setDiscription(""); // Set description
            setOrganizername1(""); // Set organizer name 1
            setOrganizername2(""); 
            setOrganizername3(""); 
            setContact1(null); 
            setContact2(null); 
            ...
            toast.info("Check the Uploaded events section to see the details of the event!.")


        } else {
            console.log("Oops sorry!, failed to upload the event")
        }

    } catch(err) {
        console.log(err)
        toast.error('All field to be filled please check and resubmit it!')
    }
  }
      ...
      <form className="form">
        ...
        <div className="page3-clublogo-eventposter">
         ...
         <div className="upload-event-btn-div">
            <center>
                <button className="upload-event-btn" onClick={handleSubmitEvent} >Upload Event</button>
            </center>
         </div>
      </form>
      <ToastContainer />
